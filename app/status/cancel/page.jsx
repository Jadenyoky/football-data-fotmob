...
export default function Cancel(p) {
  console.log(p);
  ...
  const apiMatches = async () => {
    ...
    // Cancelled Matches
    const cancelLeagues = _.filter(filter, {
      matches: [
        {
          status: {
            cancelled: true,
    ...
    setresults(cancelLeagues);

    if (control.current.value !== "") {
      setls(
        cancelLeagues.filter(
          (ele) =>
